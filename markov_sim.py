...
def run_simulation(p: np.ndarray, steps: int, count_states=False, return_state=False):
    ...
    while step < steps:
        ...
        change_probability = p[state, state]
        # print(f"State, probability: {state}, {change_probability}")
...
def run_many_simulations(p: np.ndarray, steps: int, n_simulations: int) -> float:
    ...
    while n < n_simulations:
        ...
        # Run simulation and store the last state
        result = run_simulation(p=p, steps=steps, return_state=True)
        
        # Check the last state and count it if its 0
        if result == 0: 
            count_zeros += 1
    ...
    # Compute proportion of simulations that ended with step 0
    return count_zeros / n_simulations
